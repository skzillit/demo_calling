...
import static com.example.demovideocalling.lib.JsonUtils.jsonPut;
import static com.example.demovideocalling.lib.JsonUtils.toJsonObject;
...
public class RoomClient extends RoomMessageHandler {
        ...
        () -> {
          ...
          try {
            mChatDataProducer.send(
                new DataChannel.Buffer(ByteBuffer.wrap(txt.getBytes(StandardCharsets.UTF_8)), false));
          } catch (Exception e) {
  ...
  private final Protoo.Listener peerListener =
      new Protoo.Listener() {
        @Override
        public void onOpen() {
